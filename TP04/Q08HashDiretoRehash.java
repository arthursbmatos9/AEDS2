...
class Jogador {
    ...
    public static void main(String args[]) throws Exception {
        ...
        RandomAccessFile arqLog = new RandomAccessFile("matricula_hashRehash.txt",
                "rw");
...
class TabHash {
    ...
    public TabHash() {
        this(25); // enunciado pede valores 21 e 9
    }

    public TabHash(int tam) {
        this.tam = tam; // tamanho da tabela
    ...
    public void inserir(Jogador x) {
        int pos = hash(x.getAltura());

        if (tabela[pos] == nulo) { // se tiver vazio, inserir na posicao
            tabela[pos] = x;
        } else {
            pos = rehash(x.getAltura()); // caso contrario, fazer o rehash para ver a proxima posicao
            if (tabela[pos] == nulo) {
                tabela[pos] = x;
            }
        }

        // else = NAO HA ESPACO NA TABELA
    }
    ...
    public boolean pesquisar(Jogador x) {
        ...
        int pos = hash(x.getAltura());
        if (tabela[pos] == x) { // caso esteja na posicao desejada
            existe = true;
        } else {
            pos = rehash(x.getAltura());
            if (tabela[pos] == x) { // caso contrario, pesquisar na posicao do rehash
                existe = true;
            }
        }
