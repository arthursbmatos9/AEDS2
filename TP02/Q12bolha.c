...
void bolha(struct Jogador* escolhido, int n){
    int i, j;
    for (i = 0; i < (n-1); i++) {
      for (j = 0; j < (n-1); j++) {
         if ((escolhido[j].anoNascimento > escolhido[j+1].anoNascimento) || (escolhido[j].anoNascimento == escolhido[j+1].anoNascimento && strcmp(escolhido[j].nome, escolhido[j+1].nome) > 0)){
            swap(&escolhido[j], &escolhido[j+1]);
            nMov++;
         }
          nComp++;
      }
