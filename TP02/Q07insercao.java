...
class Jogador {
    ...
    public static void ordenaInsercao(Jogador[] array, int n) {
        for (int i = 1; i < n; i++) {
            Jogador tmp = array[i];
            int j = i - 1;
                                    //fazer o WHILE enquanto o ano for maior OU enquanto o ano for igual E o nome for maior
            while (j >= 0 && (array[j].getAnoNascimento() > tmp.getAnoNascimento()) || 
                   j >= 0 && (array[j].getAnoNascimento() == tmp.getAnoNascimento() && (array[j].getNome().compareTo(tmp.getNome()) > 0))) {
                nComp++;
                array[j + 1] = array[j];
                j--;
            }
