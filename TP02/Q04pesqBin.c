...
bool pesquisaBin(struct Jogador* array, char* chave, int tamArray){
  int esq= 0, dir= tamArray-1;
  int meio;
  bool existe= false;
  ...
  while(esq <= dir)
  {
    ...
    if(strcmp(array[meio].nome, chave) == 0){
      existe = true;
      esq= tamArray;
    }
      
    else if(strcmp(array[meio].nome, chave) < 0)
      esq= meio+1;
      
    else
      dir= meio-1;

    //printf("%s\n%s\n", array[meio].nome, chave);
    //printf("%lu---%lu\n" , strlen(array[meio].nome) , strlen(chave));
  }
...
int main() {
  ...
  //NAO É NECESSARIO DAR FREE PQ NAO USA ESTRUTURA FLEXIVEL
  ...
  float inicio = (float)clock();     //pegando tempo inicial
  quickSort(escolhido, 0, tamArray-1);

  
  char* chave;
  chave = calloc(51, sizeof(char));
  fgets(chave, 50* sizeof(char), stdin);
  bool existe;
  
  while(!isFim(chave)){  //so executar se nao for FIM

      chave[strlen(chave)-1] ='\0';
      existe = pesquisaBin(escolhido, chave, tamArray);  //realizando pesquisa do nome
      if(existe == false)
        printf("NAO\n");
      else
        printf("SIM\n");
  ...
  float fim = (float)clock();      //pegando tempo final
  float tempo= fim-inicio;
  
  FILE* log= fopen("matricula_binaria.txt", "w");
  fprintf(log, "801778\t%d\t%f", nComp, tempo);
  fclose(log);
